 ...
 * Yibo Yan (ID qyan) 
 * 
 * we've add several sounds for different situation into this game
 * and we also change the original hangman picture with some cool graphics
 */
...
public class HangmanExtra extends HangmanProgram {
	
	public void run() {
		intro();		//print out the introductory information
		playBeginningSound();		//play beginning sound
		String filename = gainFilename();		//gain the file name
	...
	private void playBeginningSound() {
		AudioClip BeginningClip = MediaTools.loadAudioClip("res/Beginning.wav"); 
		BeginningClip.play(); 
	}
	...
	private int playOneGame(String secretWord) {
		...
		canvas.displayHangman(guessCount);	//display the matched picture of little hangman
		while ( hint.equals(secretWord) == false && guessCount != 0) {		//when users haven't guess out the word and haven't run out of chance
			...
			canvas.displayHangman(guessCount);		//display the new picture
	...
	/*
	 * play the matched sound
	 * when users guess the correct letter
	 */
	private void playCorrectSound() {
		AudioClip CorrectClip = MediaTools.loadAudioClip("res/Correct.wav"); 
		CorrectClip.play(); 
	}
	
	/*
	 * play the matched sound
	 * when users guess the incorrect letter
	 */
	private void playIncorrectSound() {
		AudioClip IncorrectClip = MediaTools.loadAudioClip("res/Incorrect.wav"); 
		IncorrectClip.play(); 
	}
	...
	private void playWinSound() {
		AudioClip WinClip = MediaTools.loadAudioClip("res/Win.wav"); 
		WinClip.play(); 
	}
	...
	private void playLoseSound() {
		AudioClip LoseClip = MediaTools.loadAudioClip("res/Lose.wav"); 
		LoseClip.play(); 
	}
	...
	private char readGuess(String guessedLetters) {
		...
		while (numbersOfChar == 0) {
			playIllegalInfoSound();		//when users enter the illegal info
	...
	/*
	 * play matched sound
	 * when users enter the illegal info
	 * 
	 */
	private void playIllegalInfoSound() {
		AudioClip IllegalInfoClip = MediaTools.loadAudioClip("res/IllegalInfo.wav"); 
		IllegalInfoClip.play(); 
	}
	...
	/*
	 * play matched sound
	 * when users end the game
	 */
	private void playEndSound() {
		AudioClip EndClip = MediaTools.loadAudioClip("res/End.wav"); 
		EndClip.play(); 
	}
